...
class TestBmFsInstances(unittest.TestCase):
    ...
    def test_to_json(self):
        """to_json should return a serializable dictionary object"""
        my_model_json = self.bm_obj.to_json()
        try:
            json.dumps(my_model_json)
        except (TypeError, OverflowError) as e:
            self.fail(f"Serialization failed: {e}")
        self.assertIsInstance(my_model_json, dict, "to_json return dictionary")
        self.assertIsNotNone(my_model_json, "to_json should not return None")
...
class TestGetCountFS(unittest.TestCase):
    ...
    def test_count(self):
        self.assertEqual(storage.count(), 19)
